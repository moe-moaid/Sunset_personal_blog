...
class Sunset_Profile_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		echo '<p><strong>No options for this widget!</strong><br/>You can control the fields of this widget from sunset Pofile page</p>';
	}
	...
	public function widget( $args, $instance,  ) {
		...
		?>
		<div class="text-center sunset-custom-widget">
			...
			<div class="icons-wrapper">
				<?php if( !empty( $twitter_icon ) ): ?>
					<a href="https://twitter.com/<?php echo $twitter_icon ?>" traget="_blank"><span class="sunset-icon-sidebar sunset-icon sunset-twitter"></span></a>
				<?php endif; ?>
				<?php if( !empty( $instagram_icon ) ): ?>
					<a href="https://instagram.com/<?php echo $instagram_icon ?>" traget="_blank"><span class="sunset-icon-sidebar sunset-icon sunset-instagram"></span></a>
				<?php endif; ?>
				<?php if( !empty( $facebook_icon ) ): ?>
					<a href="https://facebook.com/<?php echo $facebook_icon ?>" traget="_blank"><span class="sunset-icon-sidebar sunset-icon sunset-facebook"></span></a>
				<?php endif; ?>
			</div><!-- .icons-wrapper -->
		</div>
 ...
 */
function register_block_sunset_latest_comments() {
	register_block_type_from_metadata(
		__DIR__ . '/latest-comments',
		array(
			'render_callback' => 'render_block_sunset_latest_comments',
		)
	);
}

add_action( 'init', 'register_block_sunset_latest_comments' );
		...
		if( $posts_query->have_posts() ):
			...
			//echo '<ul>';
				...
				while( $posts_query->have_posts() ): $posts_query->the_post();
					...
					echo '<a class="sunset-popular-post-title" href="' . get_the_permalink() . '" title="' . get_the_title() . '">' . get_the_title() . '</a>';
					echo '</div>';
					//echo '<div class="row"><div class="col-xs-12 sunset-sidebar-post-meta">'. sunset_posted_footer() .'</div></div>';
